...
public class JsonSettingsManager : ISettingsManager
{
	...
	private static TSettings? Deserialize<TSettings>(FileInfo settingsFile, JsonSerializerOptions jsonSerializerOptions) where TSettings : class, ISettings, new()
	{
		...
		//var deserializedSettings = JsonSerializer.DeserializeAsync<TSettings>(stream, jsonSerializerOptions, CancellationToken.None).Result;

		using var reader = new StreamReader(stream);
		var content = reader.ReadToEnd();
		var deserializedSettings = JsonSerializer.Deserialize<TSettings>(content, jsonSerializerOptions);
